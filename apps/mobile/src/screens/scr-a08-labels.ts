...
const ko = {
  ...
  broken: (count: number) => `못 지킴 ${count}건`,
  disputed: (count: number) => `답이 달라요 ${count}건`,
  unresolved: (count: number) => `확인 못함 ${count}건`,
  active: (count: number) => `진행 중 ${count}건`,
  excluded: '답이 달라요·확인 못함은 지킴율 계산에서 제외돼요',
  reminderTitle: '알림 설정',
  remindD7: '7일 전 알림',
  remindD3: '3일 전 알림',
  remindD1: '1일 전 알림',
  remindDday: '당일 알림',
  reminderHourClose: '알림 시간 선택 닫기',
  reminderHourSelected: (time: string) => `${time} · 선택됨`,
  reminderHourTitle: '약속 알림 시간',
  reminderHour: (hour: ReminderHour) => `약속 알림 시간 ${hour}:00`,
  reminderHourValue: (hour: ReminderHour) => `${hour}:00 KST`,
  reminderHourChoice: (hour: ReminderHour) => `${hour}:00`,
  languageTitle: '언어',
  languageNames: { ko: '한국어', en: '영어' },
  // 선택 상태를 색만으로 말하지 않는다(CLAUDE.md §8-7). 라벨 문구가 상태를 함께 말한다.
  languageSelected: (name: string) => `${name} · 선택됨`,
  languageSelect: (name: string) => `${name}로 보기`,
...
const en = {
  ...
  reminderHourClose: 'Close reminder time selection',
  reminderHourSelected: (time: string) => `${time} · Selected`,
