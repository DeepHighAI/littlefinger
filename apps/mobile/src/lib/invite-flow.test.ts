...
describe('SCR-A04 초대 링크와 암호화 토큰', () => {
  ...
  test('공유 링크는 웹 기준 URL로 조립하고 공유 문구에는 제목과 링크만 넣는다', () => {
    const link = buildInviteLink('https://littlefinger.pages.dev/', invite.token);

    expect(link).toBe('https://littlefinger.pages.dev/i/raw_token-123');
    expect(inviteShareMessage(invite.title, link)).toBe(
      '주 3회 달리기\nhttps://littlefinger.pages.dev/i/raw_token-123',
    );
