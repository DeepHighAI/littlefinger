...
2. PO works through `open-testing-po-guide.md` §1–§9 and returns the §12 block. §4's production
   switch is already done; the Search Console verification file
   (`apps/web/public/googleb324b92c6f5d9c19.html`, pinned by `seo.test.ts`) is deployed and served
   with status 200, so the PO only clicks 확인 in Search Console, fixes the branding name
   (`리틀핑거`), and re-verifies branding. The public home page at `/` is deployed.
