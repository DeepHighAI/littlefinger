# 리틀핑거(Little Finger) 세부기능명세서 v1.0
...
| 상위 문서 | `기획/01_상위기획서.md` (v1.1, 2026-07-25 확정) |
| 관련 문서 | `디자인/01_와이어프레임_디자인요청서.md` (v1.0) — 화면 ID·용어 라벨의 출처 |
| 후속 문서 | `03_AI-Agent_코딩가이드` — 기술 스택 확정(N-3), 프로젝트 구조, API 시그니처, 배포 |
| 범위 제외 | API 엔드포인트 상세 스펙, 기술 스택 선정, 배포 파이프라인 (→ 03 문서) |
...
- **준종결**: `DISPUTED` — 리마인드는 없고 목록상 '완료' 탭에 표시하지만, 재협의(T-16)로 `CHECKING`에 재진입할 수 있는 유일한 예외다.
  - `DISPUTED`를 준종결로 취급하는 것은 기본안이다(S-13). 상위기획서 상태도의 `DISPUTED → COMPLETED/BROKEN`은 T-16(재협의 라운드 재개)을 거쳐 T-12/T-13으로 도달하는 것으로 구현한다.
...
- 앱: Kakao Android SDK. 카카오톡 미설치 기기는 **카카오 계정 웹 로그인으로 자동 폴백**.
- 웹: Kakao OAuth 2.0 리다이렉트 방식. 카톡 인앱 브라우저에서의 동작이 **최우선 QA 항목**(§11-1, EC-A01).
...
3. 세션: 서버 발급 JWT — access 30분 / refresh 30일(기본안). 앱은 안전 저장소(EncryptedSharedPreferences 등), 웹은 `HttpOnly; Secure; SameSite=Lax` 쿠키.
4. 신규 사용자는 `terms_agreements`에 약관 버전과 동의 시각을 기록한다(Google Play·개인정보 요건).
5. 앱 로그인 시 FCM 토큰을 `device_tokens`에 등록·갱신한다. 기기당 1행, 사용자당 최대 3기기 보관(초과 시 가장 오래된 것 삭제, EC-H04).
...
- [카카오톡으로 초대 보내기] → Kakao Link(메시지 템플릿) 공유 시트. 템플릿에는 **약속 제목만** 노출하고 내용 전문은 넣지 않는다(카톡 대화방 노출 최소화).
...
- 기한 경과 시 배치(J-03)로 전이. 이행률 계산에서 제외하고(S-8) 프로필에 "미확정 종결 n건"으로 별도 표기한다(S-15).
...
- **별도 건수 표기**(이행률 왜곡 방지, 상위기획서 F-09): "의견 불일치 n건", "미확정 종결 n건".
...
- 파기 승인 시: `CANCELED`로 전이, 미발송 리마인드 전부 취소, 이행 확인 없음, 이행률 제외.
...
| `token_hash` | char(64) | SHA-256(token). **원본 토큰 미저장** |
| `created_by` | uuid | 발송자 |
...
| `dedupe_key` | string | UNIQUE. `{promise_id}:{type}:{yyyymmdd}` |
...
| T-10 | `AMEND_PENDING` | `CANCELED` | 파기 승인 | 응답자=상대 당사자 | `closed_at`, 리마인드 전부 취소, `approvals`(CANCEL_APPROVE), 이행률 제외 |
...
> T-16은 상위기획서 상태도의 `DISPUTED → COMPLETED/BROKEN`을 구현 단위로 분해한 것이다(재협의는 CHECKING 라운드를 재사용). 결과적으로 상태도와 동일하다.
...
|---|---|---|---|---|---|---|
| NT-01 | 상대 승인 → ACTIVE | C | PUSH+INAPP | 즉시 | "{상대}님이 약속을 승인했어요" | SCR-A05 |
| NT-02 | 상대 거절 | C | PUSH+INAPP | 즉시 | "{상대}님이 약속을 거절했어요" | SCR-A05 |
...
| EC-G04 | 같은 이벤트 중복 발송 위험 | `dedupe_key = {promise_id}:{type}:{yyyymmdd}` 유니크로 1일 1회 보장 | 없음 |
...
**상위기획서·디자인요청서에 역반영이 필요한 항목**(본 문서에서 구체화하며 발생)

| 대상 문서 | 수정 필요 내용 |
|---|---|
| 상위기획서 §6 상태도 | `DISPUTED → CHECKING`(재협의 라운드 재개) 전이 명시. 현재는 `DISPUTED → COMPLETED/BROKEN`으로만 그려져 있어 구현 단위와 1:1 대응되지 않음 |
| 상위기획서 §6 UNRESOLVED 행 | 프로필 집계 라벨 "무응답 종결 n건" → "미확정 종결 n건"(S-15) |
| 상위기획서 §7 F-09 | 지킴율 산식에 keeper 한정 모수(S-1)와 최소 표본 3건(S-2) 반영 |
| 디자인요청서 §5-1 SCR-A08 | 필수요소 "이행률(%)" → "약속 지킴율(%)"(자체 용어 사전 §9 위반) |
| 디자인요청서 본문 | "패널티" 표기 → "벌칙"(자체 용어 사전 §9 위반, 6곳) |
| 디자인요청서 §8 | 면책 문구 노출 위치 3곳 → **4곳**(SCR-A08 약관 화면 추가) |

**상위기획서 잔여 이슈**: N-1(상표 검토), N-2(iOS 시점), N-3(프레임워크 확정) — 03 문서 작성 전 결정 필요.
...
본 문서 승인 후 `03_AI-Agent_코딩가이드`를 작성한다. 포함 범위: 기술 스택 확정(N-3), 프로젝트 구조, API 엔드포인트 명세, 환경변수, 에이전트 작업 순서(스프린트 분할), 테스트 전략.
