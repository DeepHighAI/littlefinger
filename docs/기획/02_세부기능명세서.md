# 리틀핑거(Little Finger) 세부기능명세서 v1.4

> PO amendment (2026-09-15, ADR 0029): invitation acceptance and witness participation
> move to the Android app after account confirmation and content review. Invitation
> web shows sender/title, Open in app and token-holder Decline without OAuth. Explicit
> anonymous partner decline ends PENDING; anonymous witness decline closes only that
> invitation slot. Unverified identity is recorded separately, never as an authenticated
> approval. This supersedes the web-login/web-acceptance and authenticated-decline
> requirements below for invitation entry; all other domain rules remain unchanged.


> **PO amendment, 2026-09-07 (ADR 0022):** The live `ads_enabled` flag is enabled
> for launch and preceding QA without the former 100-daily-confirmations threshold.
> The §11-3 seed/fallback remains `false`; it is not the live operational value.
> Exposure placement, UMP checks and independent rewarded-ad control are unchanged.
...
| v1.2 | 2026-08-29 | Added the PO-confirmed rewarded-ad and permanent-access model, per-participant record retention, no-end finish agreement, A02 in-feed banner, T-19…T-21, J-11, and NT-22/23. ADR 0015 records the rationale. |
| v1.3 | 2026-08-29 | 구현 검토 후 정합: §2-3 오류 코드 3종 추가·사문 코드 삭제, §3 화면표 광고 열(노출형/보상형 구분, MOD-04·05), §6-2 ADR 0015 테이블·컬럼, §6-3 enum 7종, §6-5 증빙 365일 규칙 폐기(증빙은 기록과 함께), §7-2 J-08 역할 축소, §8 NT-15/16 마무리·NT-22/23 조용한 시간, §9 신규 액션 9행, §10 EC-J/K/L 9건, §11-1 T-21, §11-3 5행, §13 6행. 보상형 광고를 띄울 수 없을 때는 잠금(대체 지급 없음, PO) |
| v1.4 | 2026-08-31 | PO 결정: 작성자는 승인 전 PENDING 약속을 삭제할 수 있다. 삭제는 상태를 CANCELED로 바꾸지 않고 확정 전 행·모든 초대·상대 수락 대기 항목을 원자적으로 제거한다. |
...
| 증인 | `WITNESS` | 열람 + "확인 서명"만 가능한 제3자. **판정 권한 없음** | 최대 **3명**: 작성자 기본 1 + 작성자 광고 1 + 상대방 광고 1 |
...
| `E_WITNESS_LIMIT` | 422 | 행위자에게 열린 증인 자리 소진 | "지금 사용할 수 있는 증인 자리를 모두 사용했어요." |
...
| `E_SLOT_LIMIT` | 402 | 내가 작성자인 진행 중 약속이 슬롯 용량에 도달(ADR 0009) | "약속 슬롯이 가득 찼어요. 슬롯을 추가하면 새 약속을 보낼 수 있어요." |
| `E_END_DATE_RANGE` | 402 | 발송·변경 요청의 종료일이 그 약속의 기간 상한을 넘음(ADR 0015). 슬롯 검사 뒤에 검사한다 | "설정할 수 있는 종료일 범위를 넘었어요." — 광고·구매 안내는 앱 혜택 시트(MOD-05)만 보여 준다(수락 웹 §8-1) |
| `E_REWARD_NOT_ELIGIBLE` | 422 | 보상형 광고 의도 생성·지급의 전제 조건(역할·상태·용량) 불충족 | "지금은 이 혜택을 받을 수 없어요." |
...
- 상태 라벨(화면 표시): 작성 중 / 수락 대기 / 진행 중 / 변경 대기 / 결과 확인 중 / 완료 / 불이행 / 답이 달라요 / 미확정 종결 / 거절됨 / 파기됨.
- PO-approved display-copy update (2026-09-07, ADR 0021): these labels supersede older display names throughout this specification. Status codes, transitions and policy meanings are unchanged. The `MONEY` category is displayed as `돈`.
...
| SCR-A02 | 홈·약속 목록 | APP | F-10, F-12 | 전 역할 | **허용(하단 1구좌 + 진행/대기 탭 6건 이상 시 5번째 카드 뒤 인피드 배너 1개, ADR 0015)** |
| SCR-A03 | 약속 작성 | APP | F-02 | CREATOR | 노출형 금지(P4) · 보상형 진입 허용(기간 연장 → MOD-05) |
| SCR-A04 | 초대 전송·대기 | APP | F-03 | CREATOR | 금지(P4) |
| SCR-A05 | 약속 상세 | APP | F-04·F-05·F-07·F-08·F-11 | 전 역할 | 노출형 금지(P4) · 보상형 진입 허용(기간·개인 보관 연장 → MOD-05) |
| SCR-A06 | 이행 확인 입력 | APP | F-07, F-08 | CREATOR/PARTNER | 금지(P4) |
| SCR-A07 | 알림함 | APP | F-06, F-12 | 전 역할 | **허용(1구좌, 비활성)** — 확대(PO 2026-08-24) |
| SCR-A08 | 마이·신뢰 프로필 | APP | F-09, F-12 | 본인 | **허용(1구좌, 비활성)** — 확대(PO 2026-08-24) |
| SCR-A09 | 지난 약속 히스토리 | APP | F-10 | 전 역할 | 금지 — 신설(PO 2026-08-26, ADR 0011): 홈 탭은 진행·대기 2개로 줄고 종결 6상태는 이 화면의 4탭(완료/불이행/협의 중단/거절·파기)으로 이동. 의견 불일치를 불이행으로 묶지 않는 것은 P1 |
| MOD-01 | 변경·파기·마무리 요청 모달 | APP | F-11 | CREATOR/PARTNER | 노출형 금지 · `E_END_DATE_RANGE` 시 MOD-05로 진입 |
| MOD-02 | 증인 초대 모달 | APP | F-05 | CREATOR/PARTNER | 노출형 금지 · 보상형 허용(잠긴 증인 자리 해제) |
| MOD-03 | 완료 축하 | APP | F-07 | CREATOR/PARTNER | 금지 |
| MOD-04 | 슬롯 구매 시트 | APP | F-12 | CREATOR | 금지 — ADR 0009 |
| MOD-05 | 혜택 시트(기간·개인 보관 연장, 영구 보관 구매) | APP | F-12 | CREATOR/PARTNER/WITNESS | 노출형 금지 · 보상형 허용 — 신설(ADR 0015). 광고를 띄울 수 없으면 잠긴 상태로 남는다 |
...
- **카카오 + Google SSO.** (개정 2026-08-20, PO 승인 — 원문은 카카오 단독) 그 외의 소셜 로그인·이메일 회원가입·전화번호 인증을 구현하지 않는다(전화번호 인증은 v2 강화 모드).
- **구현 경로(확정, 04 문서 §8)**: 카카오·Google 로그인 모두 **Supabase Auth의 공식 OAuth 제공자**를 경유한다. 비공식 React Native SDK는 쓰지 않는다.
- 앱: `signInWithOAuth({ provider: "kakao" })` + `expo-web-browser`의 인앱 브라우저 세션. 카카오톡 앱이 깔려 있으면 카카오가 앱으로 넘기고, **미설치 기기는 카카오 계정 웹 로그인으로 자동 폴백**된다(별도 분기 구현 불필요). 세션은 AES-256 키만 `expo-secure-store`에 두고 암호문은 AsyncStorage에 저장하는 `LargeSecureStore` 어댑터로 보관한다.
- 웹: 같은 Supabase Auth 리다이렉트 방식. 카톡 인앱 브라우저에서의 동작이 **최우선 QA 항목**(§11-1, EC-A01).
- **Explicit invitation account choice (PO 2026-09-09, ADR 0023)**: never start OAuth automatically based on the messenger. Recipients choose Google or Kakao; an existing session requires explicit account confirmation before review/acceptance. Returning from login/onboarding restores the invitation. Google login in an embedded browser provides instructions to continue the same link in a regular browser.
- **비즈 앱은 로그인 자체에 필수**다. Supabase Auth가 `account_email` scope를 포함해 요청하므로 카카오 콘솔에는 선택 동의 항목으로 등록해야 하지만, 제품은 반환된 이메일을 저장하거나 읽지 않는다.
- 수집 항목: `profile_nickname`(필수), `profile_image`(필수). `account_email`은 **선택 동의·미수집**이며 SCR-W03의 직접 이메일 입력과 이메일 발송도 MVP에서 제외한다(PO, 2026-07-29).

**4-1-2. 화면 동작**

| 화면 | 동작 |
|---|---|
| SCR-A00 온보딩 | 최초 실행 1회만. 승인된 1페이지(핵심 루프 요약) + [시작하기]/[건너뛰기]. 완료 여부는 로컬 저장. → SCR-A01. 2·3페이지는 Q-5로 보류 |
| SCR-A01 로그인 | [카카오로 시작하기]·[Google로 시작하기] 두 버튼(개정 2026-08-20). 하단에 이용약관·개인정보처리방침 링크와 "가입 시 약관에 동의한 것으로 봅니다" 고지. 로그인 성공 → 신규는 약관 동의 기록 후 SCR-A02, 기존은 SCR-A02(딥링크 진입 시 해당 화면) |
| SCR-W01 로그인 진입 | [카카오 로그인하고 내용 보기]. 로그인 성공 후 **원래 초대 토큰이 가리키던 SCR-W02로 복귀**해야 한다. 토큰은 OAuth `state` 파라미터에 담아 유지(EC-A01) |

**4-1-3. 서버 처리 규칙**

1. 로그인한 제공자의 회원 식별자(`provider`+`provider_user_id`, 개정 2026-08-20)로 `users` 조회 → 없으면 생성(`status=ACTIVE`, `primary_surface`= 로그인 표면).
...
- **증인 사용 여부** 토글을 켜면 "확정 후 증인을 초대할 수 있어요(최대 3명)" 안내를 표시한다. 실제 초대는 MOD-02에서 수행하며, 작성 화면에서 증인을 지정하지 않는다.
...
| 결과 상태 | `PENDING` → `ACTIVE` / `DECLINED` / `DRAFT`(수정 제안), 또는 작성자 삭제 |
...
| 앱 딥링크 | Android App Links로 동일 URL 처리. 앱 설치 기기는 앱에서 열고, 미설치는 웹으로(EC-I01). 카톡 인앱 브라우저는 App Links를 트리거하지 않으므로 SCR-W01이 `intent://` URI(패키지 지정 + 스토어 폴백)를 [앱에서 계속하기] CTA로 제공한다 — 설치 기기는 앱, 미설치 기기는 Play 스토어로 이동(2026-08-20 확정, ADR 0007). 웹 승인 경로는 그대로 유지한다 |

**4-3-2. SCR-A04 초대 전송·대기 (작성자)**

- [초대 링크 공유하기] → **OS 기본 공유 시트**(`expo-sharing` / RN `Share`)로 초대 링크를 보낸다(확정, 04 문서 §8). 카카오 공유 SDK의 메시지 템플릿은 MVP에서 쓰지 않는다(04 문서 §13 C-4). 공유 문구에는 **약속 제목과 링크만** 담고 내용 전문은 넣지 않는다(카톡 대화방 노출 최소화).
- 만료까지 남은 시간을 카운트다운으로 표시한다(`72:00:00` → 실시간 감소, 분 단위 갱신).
- [링크 다시 공유]: 만료 전에도 같은 링크를 다시 공유할 수 있다(토큰 재발급 없음).
- **[초대 링크 무효화]**: 잘못된 사람에게 보낸 경우 현재 PARTNER 토큰만 `REVOKED` 처리한다. 약속 상태는 `PENDING`을 유지하고 [초대 다시 보내기]로 새 토큰을 발급한다.
- **[대기 중 약속 삭제]** (PO 2026-08-31): 작성자만 실행한다. 2단계 확인 뒤 `PENDING` 약속 행을 삭제하며 현재·과거 PARTNER/WITNESS 초대, 참여 자리, 예약 알림을 같은 트랜잭션에서 함께 제거한다. 이 시점부터 상대의 수락 대기 목록과 기존 링크 모두 `E_NOT_FOUND`가 된다. 승인과 삭제가 경합하면 먼저 잠근 한쪽만 성공한다. `CANCELED`는 ACTIVE 이후 상호 파기 합의 상태이므로 사용하지 않는다.
...
- **안드로이드 UA에 한해** 주 CTA는 [앱에서 계속하기](`intent://` URI, 설치 시 앱 열기·미설치 시 Play 스토어 폴백)이고, 웹 로그인 CTA는 [웹으로 계속하기] 아래로 내려간다(2026-08-20 확정: 웹 승인 유지 + 스토어 강한 유도). iOS는 기존 그대로(EC-I03, CTA 미노출).
...
1. 확정 스탬프 + 디스클레이머.
2. **재접근 안내**: "이 약속은 카카오 로그인으로 언제든 다시 볼 수 있어요" + 북마크 안내. 초대 토큰이 소진되어도 **계정 기반으로 SCR-W04에 접근 가능**하다는 점이 핵심이다. 이메일 입력 UI는 만들지 않는다.
3. 앱 설치 유도 배너: "다음엔 직접 약속을 만들어보세요" + Play 스토어 링크(설치 전환 KPI 측정용 UTM/Referrer 부착).
...
| 화면 | 동작 |
|---|---|
| MOD-02 증인 초대 모달 | 현재 열린 자리와 사용 수, 초대 중·참여 중 증인 목록을 표시한다. 작성자는 기본 1자리와 보상형 광고 1자리, 상대방은 보상형 광고 1자리를 독립적으로 사용한다. 자기 자리가 소진되면 초대 버튼을 비활성화하며, 광고로 아직 해제할 수 있으면 [광고 보고 증인 1명 추가]를 표시한다. |
| SCR-W05 증인 확인 | 약속 전문 **읽기 전용**, 참여자 정보, 확정 시각, 권한 안내("증인은 내용을 확인하는 역할이에요. 약속이 지켜졌는지 판정하지는 않아요."), [확인 서명]. 서명 후 재방문 시 서명 시각 표시 |

- 증인 초대는 작성자와 상대방 **둘 다** 발송할 수 있다. 자리는 발송자별로 귀속되며 증인이 나가면 그 발송자의 자리가 다시 열린다. 배포 전에 이미 3명을 초과한 약속은 기존 증인을 유지하되 신규 초대만 막는다.
...
| 채널 | APP: Expo 푸시(FCM 경유) + 인앱 알림함 / WEB: 로그인 후 화면 내 표시. 이메일은 MVP 미사용 |
| 전체 매트릭스 | §8 |
...
1. 승인 완료와 재방문 화면에서 카카오 로그인 기반 재접근 경로를 분명히 안내한다.
2. **작성자 화면에 대리 알림 버튼**: 상대가 앱 기기 토큰이 없으면 SCR-A05에 "상대에게 카톡으로 알리기" 버튼을 노출해 작성자가 카카오 공유로 직접 알릴 수 있게 한다(EC-G01).
3. 이메일 수집·검증·bounce 처리는 MVP에서 구현하지 않는다(PO, 2026-07-29; EC-G03은 비적용 결정 검증 항목).
...
| 처리 | 서버에서 장변 최대 1920px 리사이즈 + EXIF **전체 메타데이터 제거**(개인정보 보호), 썸네일 320px 생성 |
| 공개 범위 | 해당 약속의 당사자 2명 + 증인(최대 3명). **그 외 접근 불가** |
| 저장 | 비공개 버킷 + 만료형 서명 URL(유효 10분, 기본안) |
| 보관 기간 | 참여자 중 마지막 유효 개인 열람권이 끝날 때 약속 상세와 함께 영구 삭제. 개인별 만료 전에는 다른 참여자의 증빙 접근에 영향을 주지 않는다. |
...
- **광고 슬롯**: 목록 하단 네이티브 1구좌를 유지한다. 진행 또는 대기 탭의 전체 행이 6건 이상이면 히어로를 포함한 다섯 번째 약속 카드 뒤에 띠배너 1개를 추가한다. `ads_enabled=false` 또는 광고 로드 실패 시 빈 공간도 남기지 않는다.
- FAB [약속 만들기] → SCR-A03.
...
- 변경 요청 시 **종료일 검증**: 새 종료일은 **요청 시점 KST 오늘 포함 이후**여야 한다. 승인 시점에 다시 검증하며, 그 사이 지나버렸으면 승인을 차단하고 재요청을 안내한다(EC-E03).
...
| 항목 | 내용 |
|---|---|
| 관련 화면 | SCR-A02·SCR-A07·SCR-A08 하단 **각 1구좌** (A07·A08 은 PO 2026-08-24 확대, ADR 0009) |
| 형식 | 하단 네이티브 광고 + A02 띠배너 + 사용자가 직접 시작하는 보상형 광고(AdMob 계열) |
| 활성화 조건 | **일 확정(ACTIVE 전환) 약속 100건 도달**(확정) |
| 제어 | 서버 원격 플래그 `ads_enabled`(기본값 `false`). 앱 업데이트 없이 켤 수 있어야 한다 |

**금지 구역(P4, 코드 레벨 강제)**: 배너/네이티브 노출형 광고는 SCR-A03 작성, SCR-A04 초대, SCR-A05 상세, SCR-A06 이행 확인, MOD-01~03, **수락용 웹 전체(SCR-W01~W06)**에서 금지한다. 사용자가 혜택을 선택한 보상형 광고는 앱의 기간·보존·증인 해제 UI에서만 허용한다. 광고 SDK는 앱 모듈에만 포함하고 **웹 프로젝트에는 광고 라이브러리를 아예 추가하지 않는다.**
...
- 임계값 판단: ACTIVE 전환 트랜잭션이 `daily_metrics.activated_count`를 실시간 증가시킨다. **자동 활성화하지 않는다** — 운영자가 수치를 확인하고 PO 결정으로 플래그를 켠다. J-07 자동 집계·운영자 경보는 현재 범위에서 보류됐다.
- 보상은 클라이언트의 `EARNED_REWARD` 이벤트로 부여하지 않는다. 서버가 발급한 intent를 AdMob SSV `custom_data`에 넣고, Google ECDSA 서명·허용 광고 단위·거래 ID 재사용을 서버에서 확인한 뒤에만 지급한다.
- 보상형 광고를 띄울 수 없으면(플래그 off·no-fill·동의 거부·로드 시간 초과) 혜택은 **잠긴 상태로 남는다**(PO 2026-08-29). 서버는 no-fill을 검증할 수 없으므로 무료 대체 지급은 두지 않는다. 상품이 있는 혜택은 `promise_permanent_access` 구매만 대안이다.
- 유료 사용자도 광고 제거 대상이 아니다. `ads_enabled`와 `rewarded_ads_enabled`는 서로 독립이다.
...
| 3 | 카테고리 | `category` | enum | 선택 — 미선택 시 `ETC`(기타)로 저장(PO 2026-08-26 개정, ADR 0011) | `HABIT`(습관) / `BET`(내기) / `MONEY`(금전) / `ETC`(기타) | 없음 | `DRAFT` / 변경 합의 |
| 4 | 종료일 | `end_date` | date nullable | 선택(무기한은 NULL) | KST 오늘부터 선택 가능(PO 2026-09-14, ADR 0026). 작성일 +30일까지 무료. 작성자 보상형 광고 1회마다 해당 약속 상한 +30일. 작성자 `promise_permanent_access`가 유효하면 NULL(종료일 없음) 또는 장기 날짜 가능. 발송·변경 요청 시 서버 재검증 | 없음 | `DRAFT` / 변경 합의(승인 시점 재검증) |
...
| 3 | 증빙 사진 | `evidences[]` | file[] | — | 최대 3장, 장당 5MB, JPEG/PNG/WEBP/HEIC | 위와 동일(개별 삭제 가능) |
...
> SCR-W03 이메일 필드는 PO 결정(2026-07-29)으로 MVP에서 제거됐다. 카카오 OAuth 응답 이메일도 저장·사용하지 않는다.
...
|---|---|---|---|
| `id` | uuid | PK | |
| `provider_user_id` | string | NOT NULL, UNIQUE(provider, provider_user_id) | SSO 제공자 회원 식별자(개정 2026-08-20, 구 `kakao_id`). 탈퇴 시 해시로 대체 |
| `provider` | string | NULL | `kakao`·`google`. 프로비저닝 전(pending)에는 NULL |
| `nickname` | string(40) | NOT NULL | 카카오 닉네임. 로그인마다 동기화 |
| `profile_image_url` | string | NULL | |
| `email` | string | NULL | 레거시 호환 컬럼. MVP에서는 항상 NULL이며 읽거나 쓰지 않음 |
| `email_verified` | boolean | DEFAULT false | 레거시 호환 컬럼. MVP에서 사용하지 않음 |
| `email_bounce_count` | int | DEFAULT 0 | 레거시 호환 컬럼. MVP에서 사용하지 않음 |
...
**`approvals`** — 감사 추적 로그. **확정 기록은 append-only, 삭제 불가**(계정 탈퇴에도 보존). 단 확정되지 않은 수정 제안 DRAFT가 J-06의 90일 수명주기로 삭제될 때 그 DRAFT의 `AMEND_SUGGEST` 로그는 부모와 함께 제거한다.
...
| `type` | enum | §8-1 NT 코드 |
| `channel` | enum | `PUSH`/`EMAIL`/`INAPP`. `EMAIL`은 레거시 enum 값이며 MVP fanout에서는 생성하지 않음 |
| `title` / `body` / `deeplink` | string | |
| `status` | enum | `QUEUED`/`SENT`/`FAILED`/`READ` |
| `fail_reason` | string | Expo/FCM 무효 토큰 등 |
...
**2026-08-29 추가 (ADR 0015)** — 이 배치가 추가한 테이블·컬럼. 모두 서버 전용(RLS + 3중 revoke)이며 클라이언트는 `promise-entitlements` 함수가 계산한 결과만 본다.

| 테이블 / 컬럼 | 역할 |
|---|---|
| `promise_participants.invited_by_user_id` | 증인 자리의 소유자(초대한 당사자). 역할별 용량 계산과 자리 반환의 기준 |
| `promises.retention_anchor_at` / `promises.purge_state` | 개인 보관 시계의 기준 시각 / `AVAILABLE`·`PURGING` (§2-4 상태값과 별개) |
| `slot_purchases.promise_id` | `promise_permanent_access` 구매가 붙는 약속. 슬롯 상품은 NULL. 약속이 삭제되면 NULL로 남아 환불 원장을 보존한다 |
| `promise_duration_baselines` | 약속별 기간 상한의 기준일(KST 작성일) |
| `reward_intents` | 보상형 광고 의도. `opaque_user_id`(sha256)와 SSV `custom_data`=intent id로 Google 콜백을 묶는다. TTL `REWARD_INTENT_TTL_MIN` |
| `promise_reward_grants` | append-only 지급 원장. `transaction_id` unique가 SSV 재생을 막는다. source는 `ADMOB_SSV`·`MIGRATION` — 대체 지급 source는 없다 |
| `promise_access_graces` | 배포 시 부여한 참여자별 유예 만료 시각 |
| `user_keep_rate_aggregates` | 정리(purge)된 약속의 비식별 지킴율 집계. `lf_recompute_trust_profile`이 합산한다 |
| `purged_promise_receipts` | 집계 이관 영수증 — 같은 약속을 두 번 합산하지 않는 멱등 키 |
| `promise_purge_jobs` | 정리 작업 큐. `PENDING`·`LEASED`, lease id로 finalize를 펜싱한다 |
...
ApprovalAction: APPROVE | DECLINE | AMEND_SUGGEST | AMEND_REQUEST | AMEND_APPROVE | AMEND_DECLINE |
                AMEND_WITHDRAW | CANCEL_REQUEST | CANCEL_APPROVE | CANCEL_DECLINE | WITNESS_SIGN |
                FINISH_REQUEST | FINISH_APPROVE | FINISH_DECLINE | FINISH_WITHDRAW
AmendStatus   : PENDING | APPROVED | DECLINED | WITHDRAWN | EXPIRED
UserStatus    : ACTIVE | SUSPENDED | WITHDRAWN
RewardAction  : WITNESS_CREATOR | WITNESS_PARTNER | DURATION_30D | RETENTION_30D
RewardIntentStatus : PENDING | GRANTED | REJECTED
RewardGrantSource  : ADMOB_SSV | MIGRATION
PurgeState    : AVAILABLE | PURGING
PurgeJobStatus: PENDING | LEASED
```
...
| 증빙 이미지 | 기록과 함께 산다 — 마지막 유효 열람권이 끝나 기록이 정리될 때(J-11), 또는 첨부한 사람이 제거(`removed_at`)했을 때만 삭제(PO 2026-08-29: 종결 후 365일 자동 삭제 폐기). 제거 후 "만료된 증빙" 표시 |
| 알림 | 90일 경과 시 삭제(기본안) |
| IP·User-Agent | 원본 미저장, salt 해시만 보관 |
| 계정 탈퇴 | `status=WITHDRAWN`, `withdrawn_at` 기록 + **개인정보 비식별화**: `nickname`→"탈퇴한 사용자", `profile_image_url`→NULL, `email`→NULL, `provider_user_id`→`sha256(provider_user_id + ACCOUNT_ID_PEPPER)`로 대체. `user_id`와 참여·승인 이력은 유지해 **상대방의 기록 열람권 보존** |
...
| 재가입 | 동일 SSO 계정이어도 **신규 `user_id`**. 이전 약속·지킴율은 승계하지 않는다 |

아래 2026-08-29 보관 규칙이 위 표의 "삭제 불가"·증빙 행과 충돌하면 아래가 우선한다:

| 대상 | 확정 정책 |
|---|---|
| 성립한 유한 약속 | `end_date` 다음 날 00:00 KST를 anchor로 각 JOINED 참여자에게 30일 무료 열람권. CHECKING/DISPUTED 여부와 무관하게 anchor는 바뀌지 않는다. |
| 성립한 종료일 없는 약속 | 한 당사자가 종료를 요청하고 상대가 승인한 서버 시각을 anchor로 CHECKING에 진입하고, 각 참여자에게 30일 무료 열람권을 준다. |
| 개인 연장 | 당사자·증인 각자가 보상형 광고 1회당 자기 열람권만 +30일. 반복 가능. ₩2,000 `promise_permanent_access`는 구매자 자기 열람권만 영구화한다. |
| 만료 | 본인 열람권 만료 즉시 목록·상세에서 사라지며 광고·구매로 복원할 수 없다. 다른 참여자의 유효 열람권은 영향을 받지 않는다. |
| 물리 삭제 | JOINED 참여자 중 누구도 유효 열람권 또는 유효 영구보존을 갖지 않으면 `promises.purge_state = 'PURGING'`으로 전환한다(§2-4 상태값은 바뀌지 않는다). 스토리지 객체를 먼저 지우고 상세·버전·승인·이행·증빙 행을 영구 삭제한다. 상태별 지킴율 집계만 비식별 aggregate에 이관한다. |
| 미성립 기록 | DRAFT는 마지막 수정 +90일. PENDING은 마지막 초대 만료 +30일. 한 번도 ACTIVE가 아니었던 DECLINED/CANCELED는 `closed_at +30일`. 개인 보존 상품/광고 대상이 아니다. |
| 기존 데이터 | 배포 시 이미 새 규칙상 만료된 성립 기록은 참여자별 배포 +30일 grace를 준다. 기존 장기 종료일과 기존 증인 수는 그대로 인정한다. |
| 환불 | 개인 영구보존만 철회하고 무료/광고 만료 시각으로 재계산한다. 이미 합의된 종료일 없음과 종료 합의는 되돌리지 않는다. |
...
| T-19 | `ACTIVE` | `AMEND_PENDING` | 종료일 없는 약속의 [약속 마무리 요청] | 요청자=당사자, `end_date IS NULL`, PENDING 요청 없음 | `amend_requests(type=FINISH)`, `approvals(FINISH_REQUEST)`, 상대 승인 요청 |
| T-20 | `AMEND_PENDING` | `CHECKING` | 종료 요청 승인 | 응답자=상대 당사자, 요청 유효 | 승인 서버 시각을 `retention_anchor_at`과 `checking_started_at`에 동일 기록, +7일 기한과 CHECK_REQ/R1/R2 예약, `approvals(FINISH_APPROVE)` |
| T-21 | `AMEND_PENDING` | `ACTIVE` | 종료 요청 거절·철회·기한 만료 | FINISH 요청 | 기존 무기한 ACTIVE로 복귀, `FINISH_DECLINE` 또는 `FINISH_WITHDRAW` 감사 로그 |

> `CHECKING → AMEND_PENDING`, `ACTIVE → COMPLETED`(직접), `COMPLETED → *`, `UNRESOLVED → *`, `CANCELED → *`, `DECLINED → *` 전이는 **존재하지 않는다.** `AMEND_PENDING → CHECKING`은 T-20의 FINISH 승인에만 존재한다.
> T-16은 상위기획서 v1.2 상태도의 `DISPUTED → CHECKING`과 동일한 전이다(재협의는 CHECKING 라운드를 재사용). 종결은 재진입한 라운드에서 T-12/T-13으로 도달한다.
> PENDING 작성자 삭제는 DRAFT 삭제와 같은 **확정 전 행 삭제**이며 다른 `PromiseStatus`로 가는 전이가 아니다. 따라서 T-ID를 추가하지 않고, 삭제 후 연결된 초대·수락 대기 항목은 존재하지 않는다(PO 2026-08-31).
...
| J-06 DRAFT 정리 | 매일 04:00 | 수정 제안 DRAFT에 +3일 NT-20, 모든 DRAFT에 삭제 7일 전 NT-21 예약. 90일 미수정이고 NT-21 발송이 끝난 `DRAFT`만 삭제 |
| J-07 일 지표 집계 | **현재 범위 보류** | PO 결정으로 MVP 로컬 완성 범위에서 제외. `daily_metrics.activated_count`는 ACTIVE 전환 시 실시간 기록하며 광고 플래그를 자동으로 켜지 않는다 |
| J-08 증빙 정리 | 매주 일 05:00 | 첨부자가 제거한(`removed_at`) 증빙 객체만 스토리지에서 삭제. 종결 후 365일 자동 삭제(`purge_after`)는 2026-08-29 폐기 — 증빙은 기록과 함께 J-11이 정리한다 |
...
| J-11 보존 관리 | 매시 17분 | 개인 보존 만료 D-7/D-1 알림을 멱등 생성하고 마지막 유효 열람권이 끝난 약속을 purge queue에 넣는다. Edge worker가 증빙 스토리지 삭제 후 DB 삭제를 finalize한다. |
...
수신자 표기: **C**=작성자, **P**=상대방, **W**=증인. 채널: PUSH(앱), INAPP(알림함). EMAIL은 MVP에서 사용하지 않는다.
...
| NT-06 | 리마인드 D-7/D-3/D-1 | C, P | PUSH+INAPP(앱) / INAPP(웹) | 09:00 | "약속까지 {n}일 남았어요" | SCR-A05 / SCR-W04 |
| NT-07 | 리마인드 D-Day | C, P | PUSH+INAPP(앱) / INAPP(웹) | 09:00 | "오늘이 약속 종료일이에요" | SCR-A05 / SCR-W04 |
| NT-08 | CHECKING 진입 | C, P | PUSH+INAPP(앱) / INAPP(웹) | 09:00 | "약속이 지켜졌나요?" | SCR-A06 / SCR-W04 |
| NT-09 | 상대가 먼저 응답 | 미응답자 | PUSH+INAPP(앱) / INAPP(웹) | 즉시 | "{상대}님이 이행 확인을 보냈어요" | SCR-A06 / SCR-W04 |
| NT-10 | 이행 확인 미응답 독촉(+2일, +5일) | 미응답자 | PUSH+INAPP(앱) / INAPP(웹) | 09:00 | "이행 확인이 {n}일 남았어요" | SCR-A06 / SCR-W04 |
| NT-11 | COMPLETED | C, P, W | PUSH+INAPP(앱) / INAPP(웹) | 즉시 | "약속을 지켰어요!" | SCR-A05 |
| NT-12 | BROKEN | C, P, W | PUSH+INAPP(앱) / INAPP(웹) | 즉시 | "약속이 불이행으로 기록됐어요" | SCR-A05 |
| NT-13 | DISPUTED | C, P, W | PUSH+INAPP(앱) / INAPP(웹) | 즉시 | "두 분의 확인이 서로 달라요" | SCR-A05 |
| NT-14 | UNRESOLVED | C, P | PUSH+INAPP(앱) / INAPP(웹) | 즉시 | "이행 확인 없이 종결됐어요" | SCR-A05 |
| NT-15 | 변경·파기·마무리 요청 도착 | 상대 당사자 | PUSH+INAPP(앱) / INAPP(웹) | 즉시 | "{상대}님이 약속 {변경/파기/마무리}을 요청했어요" | SCR-A05 / SCR-W04 |
| NT-16 | 변경·파기·마무리 응답(승인/거절) | 요청자 | PUSH+INAPP(앱) / INAPP(웹) | 즉시 | "요청이 {승인/거절}됐어요" | SCR-A05 |
| NT-17 | 변경 요청 기한 만료 자동 철회 | C, P | PUSH+INAPP(앱) / INAPP(웹) | 즉시 | "변경 요청이 자동 철회됐어요" | SCR-A05 |
| NT-18 | 증인 확인 서명 완료 | C, P | PUSH+INAPP(앱) / INAPP(웹) | 즉시 | "{증인}님이 내용을 확인했어요" | SCR-A05 |
| NT-19 | 재협의 요청 | 상대 당사자 | PUSH+INAPP(앱) / INAPP(웹) | 즉시 | "다시 확인해 달라는 요청이 왔어요" | SCR-A06 / SCR-W04 |
| NT-20 | 수정 제안 뒤 작성 중 DRAFT 방치 | C | PUSH+INAPP | 수정 초안 갱신 +3일 | "작성 중인 약속이 있어요" | SCR-A03 |
| NT-21 | DRAFT 자동 삭제 예고 | C | PUSH+INAPP | 마지막 수정 +83일 | "초안이 7일 뒤 삭제돼요" | SCR-A03 |
| NT-22 | 개인 보존 만료 D-7 | 해당 참여자 | PUSH+INAPP | 만료 7일 전 | "약속 기록이 7일 뒤 정리돼요" | SCR-A05 |
| NT-23 | 개인 보존 만료 D-1 | 해당 참여자 | PUSH+INAPP | 만료 1일 전 | "약속 기록이 내일 정리돼요" | SCR-A05 |

- **초대 발송 자체는 시스템 알림이 아니다.** 작성자가 원하는 메신저로 초대 링크를 직접 공유한다(알림톡 MVP 미도입, 확정).
...
| DRAFT_RESUME | 수정 제안으로 DRAFT 회귀 | 마지막 수정 +3일. 1회만 생성 |
| DRAFT_DELETE_SOON | DRAFT 생성·수정 | 마지막 수정 +83일. 실제 삭제 전 발송 완료 필수 |
| RETENTION_D7 / RETENTION_D1 | 개인 만료 시각 확정·연장 시 | 각 참여자의 `expires_at - 7일 / -1일`. 영구보존 또는 이미 만료된 경우 생성하지 않음 |
...
- **조용한 시간(21:00~08:00 KST)**: NT-04, NT-06~NT-08, NT-10, NT-20~NT-23(스케줄 알림)은 다음 08:00으로 이연. NT-01~NT-03, NT-09, NT-11~NT-19(즉시성 알림)는 예외로 즉시 발송한다.
- 발송 실패 시 `notifications.status=FAILED` + `fail_reason` 기록. FCM 무효 토큰은 `device_tokens`에서 삭제한다.
- 재시도: PUSH 3회(지수 백오프). 초과 시 INAPP만 유지한다. 이메일 fanout은 만들지 않는다.
...
| 신고·차단 | ✅ | ✅ | ✅ | ❌ |
| 기록 삭제 | ❌ | ❌ | ❌ | ❌ |
| 보상형 광고 의도 생성·상태 조회 | △(JOINED·전제 조건 충족) | △(동일) | △(개인 보관 연장만) | ❌ |
| 기간 상한 연장(광고) · 종료일 없음 제안 | △(작성자만, 제안은 영구 구매 후) | ❌ | ❌ | ❌ |
| 개인 보관 연장(광고) | △(성립 후·본인 만료 전) | △(동일) | △(동일) | ❌ |
| 영구 보관 구매(`promise_permanent_access`) | △(JOINED) | △(JOINED) | △(JOINED) | ❌ |
| 마무리 요청(종료일 없는 ACTIVE) | △ | △ | ❌ | ❌ |
| 마무리 승인·거절 | △(상대 요청 시) | △(상대 요청 시) | ❌ | ❌ |
| 마무리 철회 | △(요청자) | △(요청자) | ❌ | ❌ |
| 본인 열람권 만료 후 열람 | ❌(`E_NOT_FOUND`) | ❌ | ❌ | ❌ |
| 정리 중(PURGING)·정리된 기록 열람 | ❌ | ❌ | ❌ | ❌ |
...
| EC-A05 | 앱에서 로그인한 계정과 웹에서 로그인한 계정이 서로 다름(가족 기기 등) | 계정 식별은 `provider`+`provider_user_id` 기준(개정 2026-08-20). 다른 계정이면 별개 사용자로 처리. 초대 대상 역할이 이미 채워져 있으면 `E_DUPLICATE_ROLE` | "이 약속에는 이미 다른 분이 참여하고 있습니다." |
...
| ID | 상황 | 시스템 동작 | 사용자 노출 |
|---|---|---|---|
| EC-D01 | 발송자의 증인 자리 소진 또는 전체 상한(3명) 도달 | `E_WITNESS_LIMIT`. 초대 생성 자체를 차단 | "지금 사용할 수 있는 증인 자리를 모두 사용했어요." 광고 해제가 남아 있으면 해당 CTA 표시 |
...
| EC-E03 | 변경으로 제안된 종료일이 과거 | `E_VALIDATION`. 제안 자체를 저장하지 않음 | "종료일은 오늘 이후의 날짜로 정해주세요." |
...
| EC-F06 | 부적절한 이미지 신고 접수 | 해당 증빙 `blinded_at` 설정 → 즉시 비노출. 약속 상태는 불변 | 열람 시: "신고 접수로 가려진 이미지입니다" |
...
| ID | 상황 | 시스템 동작 | 사용자 노출 |
|---|---|---|---|
| EC-G01 | 웹 참여자에게 앱 기기 토큰이 없음 | 인앱 알림함에 적립하고 작성자 화면에 "상대에게 카톡으로 알리기" 버튼 노출 | 작성자: "상대는 앱 알림을 받을 수 없어요. 직접 알려주세요." |
| EC-G02 | 푸시 토큰 만료·기기 초기화 | FCM 실패 응답 시 `device_tokens` 행 삭제. 남은 채널로 발송 | 없음 |
| EC-G03 | 이메일 발송 경로 진입 | **MVP 비적용**. 이메일 주소를 수집하지 않고 EMAIL fanout·bounce 처리 코드를 호출하지 않는다(PO, 2026-07-29) | 이메일 입력·재입력 문구 없음 |
...
| ID | 상황 | 시스템 동작 | 사용자 노출 |
|---|---|---|---|
| EC-I01 | 앱 설치된 안드로이드 기기에서 초대 링크 탭 | App Links로 앱이 열리고 **앱 안에서 초대 검토·승인·거절·수정 제안까지 완료**(2026-08-20 실구현, ADR 0007). 승인 시 `approvals.surface = 'APP'` 기록(RN fetch는 Origin 미전송 — 서버가 그것으로 판정). 카톡 인앱 브라우저는 App Links 미트리거 → SCR-W01의 [앱에서 계속하기] intent CTA가 같은 진입을 만든다. 실패 시 웹으로 폴백. **증인(WITNESS) 토큰은 예외**: 앱은 브라우저 핸드오프 카드를 보여 주고 확인은 웹(SCR-W05)에서 한다 | 자연스러운 앱 전환 |
...
| EC-I04 | 앱 강제 업데이트 필요(서버 최소 버전 상향 — 2026-08-29 배치는 증인 목록·홈·상세 응답 형태가 바뀌어 `0.2.0`으로 상향한다) | `app_configs.min_app_version` 비교 후 차단 화면 | "업데이트 후 이용해 주세요." + [스토어로 이동] |

### 10-10. EC-J: 보상형 광고·혜택 (ADR 0015)

| ID | 상황 | 시스템 동작 | 사용자 노출 |
|---|---|---|---|
| EC-J01 | PENDING 의도가 살아 있는데 같은 약속·행동으로 다시 요청 | 새 행을 만들지 않고 살아 있는 의도를 그대로 돌려준다 | 광고 재생 |
| EC-J02 | SSV 콜백이 같은 `transaction_id`로 다시 도착 | 같은 의도의 재전송은 멱등 확인(`granted:true`)으로 답하되 지급 행은 1개 그대로, 다른 의도가 같은 `transaction_id`를 쓰면 `granted:false` | 없음 |
| EC-J03 | 상대방이 변경 요청으로 작성자 기간 상한 밖 종료일을 제안 | `E_END_DATE_RANGE`. 상한은 작성자만 늘릴 수 있다 | "설정할 수 있는 종료일 범위를 넘었어요." |
| EC-J04 | 내가 나갔거나 참여하지 않은 약속의 영구 보관 구매 검증 | `E_NOT_FOUND` — 존재조차 알리지 않는다(§9). 스토어 구매는 소비하지 않고 다음 시트 열림에 재검증한다 | 결제 확인 실패 안내 |

### 10-11. EC-K: 개인 보관·정리 (ADR 0015)

| ID | 상황 | 시스템 동작 | 사용자 노출 |
|---|---|---|---|
| EC-K01 | 본인 열람권 만료 뒤 광고 연장·영구 구매 시도 | 복원 불가 — 광고 의도는 `E_REWARD_NOT_ELIGIBLE`, 구매 검증은 기록이 없는 것으로 답한다(`E_NOT_FOUND`) | 기록이 보이지 않음 |
| EC-K02 | 마무리 요청이 대기 중인 종료일 없는 약속에 J-11 실행 | anchor가 없으므로 경고도 정리 대상도 아니다 | 없음 |

### 10-12. EC-L: 종료일 없는 약속·마무리 (ADR 0015)

| ID | 상황 | 시스템 동작 | 사용자 노출 |
|---|---|---|---|
| EC-L01 | 종료일이 있는 약속에 마무리 요청 | `E_STATE_CONFLICT` — 유한 약속은 J-02가 종료일에 CHECKING으로 보낸다 | "약속 상태가 변경됐어요…" |
| EC-L02 | 다른 변경·파기 요청이 대기 중일 때 마무리 요청 | `E_STATE_CONFLICT` — 요청은 한 번에 하나 | 동일 |
| EC-L03 | 종료일 없는 ACTIVE 약속에 증인 초대 | 허용 — 증인 용량은 종료일과 무관 | 없음 |
...
| 등급 | 대상 | 기준 |
|---|---|---|
| P0 (출시 차단) | 확정 트랜잭션(§4-3-5), 상태 전이 T-01~T-21, 권한 매트릭스(§9), 기록 불변성(P3) | 1건이라도 실패하면 출시 불가 |
...
| `WITNESS_MAX` | 3 | **확정**(PO 2026-08-29) |
| `WITNESS_CREATOR_FREE` / `WITNESS_REWARDED_PER_ROLE` | 1 / 1 | **확정**(PO 2026-08-29) |
...
| `EVIDENCE_MAX_COUNT` / `EVIDENCE_MAX_MB` | 3 / 5 | 기본안 |
| `DRAFT_MAX_CONCURRENT` / `PROMISE_MAX_PER_DAY` | 20 / 30 | 기본안 |
| `INVITE_RESEND_MAX` | 10 | 기본안 |
| `DEVICE_TOKEN_MAX` | 3 | 기본안 |
| `END_DATE_FREE_DAYS` / `END_DATE_EXTENSION_DAYS` | 30 / 30 | **확정**(PO 2026-08-29) |
| `END_DATE_MAX_DAYS` | 36500 | 기술적 date-picker ceiling; business range is server entitlement | 구현 상한 |
| `RETENTION_FREE_DAYS` / `RETENTION_EXTENSION_DAYS` | 30 / 30 | **확정**(PO 2026-08-29) |
| `PERMANENT_ACCESS_PRODUCT_ID` / 기본 표시가 | `promise_permanent_access` / ₩2,000 | **확정**; 스토어 현지화 가격이 정본 |
| `HOME_BANNER_AFTER_PROMISES` | 5 | **확정**(6건 이상일 때 5번째 카드 뒤) |
| `RETENTION_WARNING_DAYS` | [7, 1] | 기본안 — SQL 정본 `lf_retention_warning_days()`, 테스트가 대조 |
| `REWARD_INTENT_TTL_MIN` | 15 | 기본안 — SQL 정본 `lf_reward_intent_ttl_minutes()`; SSV는 서명된 timestamp로 판정 |
| `REWARD_SSV_WAIT_MS` / `REWARD_SSV_POLL_MS` | 15000 / 2000 | 기본안(앱 전용 — 광고 뒤 SSV 결과 대기·조회 간격) |
| `REWARD_SHOW_TIMEOUT_MS` | 20000 | 기본안(앱 전용 — 광고 로드·표시 무응답 시 잠금 처리) |
| `DRAFT_TTL_DAYS` | 90 | 기본안 |
| `AMEND_MAX_ROUNDS` | 무제한 | 기본안(S-10) |
| `ads_enabled` (원격 플래그) | false | **확정**(PO 수동 전환) |
| `rewarded_ads_enabled` (원격 플래그) | true | **확정**; 노출형 광고 플래그와 독립 |
...
| ~~S-6~~ | ~~이메일 수집 시점~~ | **종료: MVP 미수집·미발송**(PO, 2026-07-29) | 웹 참여자는 카카오 로그인 재접근 + 작성자의 카카오톡 직접 알림으로 보완 |
| ~~S-7~~ | ~~종료일 최대 범위~~ | **종료: 작성일 +30일 무료, 광고당 +30일, 작성자 영구 상품으로 무제한**(PO 2026-08-29) | ADR 0015 |
...
| ~~S-11~~ | ~~PENDING 철회를 "초대 링크 무효화"로만 제공~~ | **종료: 현재 링크만 무효화와 PENDING 약속 삭제를 모두 제공**(PO, 2026-08-31) | 삭제는 새 상태를 추가하지 않고 초대·상대 수락 대기와 함께 하드 삭제 |
...
- [ ] 작성자가 PENDING 약속을 삭제하면 발송이 취소되고 모든 기존 링크가 무효해지며 상대 수락 대기 목록에서도 즉시 사라진다.
...
- [ ] 작성자는 증인 1명을 무료 초대하고 광고로 1명만 추가하며, 상대방도 광고로 자기 자리 1명을 독립적으로 추가한다.
- [ ] 작성일 +30일을 넘는 종료일은 `E_END_DATE_RANGE`이고 SSV 광고 1회마다 +30일, 작성자 영구 상품 후에는 종료일 없음 제안이 가능하다.
- [ ] 종료일 없는 약속은 한쪽 종료 요청과 상대 승인 뒤에만 CHECKING으로 들어가며 승인 시각이 개인 보존 anchor가 된다.
- [ ] 개인 열람권은 참여자별로 만료·연장·영구화되고, 본인 만료 후 복원할 수 없으며 마지막 유효 열람권 종료 뒤 상세와 스토리지가 멱등 삭제된다.
- [ ] 보존 만료 D-7/D-1 알림은 수신자별 1회이고, purge 뒤에도 비식별 약속 지킴율 aggregate는 유지된다.
- [ ] ACTIVE/WAITING 탭 6건 이상이면 히어로 포함 다섯 번째 카드 뒤 배너 1개, `ads_enabled=false` 또는 no-fill이면 빈 공간 없이 0개다.
- [ ] 보상형 광고를 띄울 수 없으면(플래그 off·no-fill·동의 거부·시간 초과) 혜택은 잠긴 채 남고, 어떤 경로로도 무료 지급이 생기지 않는다.
- [ ] 영구 보관 구매 환불(voided)은 구매자 개인 열람권만 회수하고, 이미 합의된 종료일 없음·마무리 합의를 되돌리지 않는다.
- [ ] 앱·웹 모두에서 마무리 요청 → 상대 승인/거절 → 요청자 철회가 대칭으로 동작하고 NT-15/NT-16이 도착한다.
- [ ] 0.1.x 빌드는 `min_app_version` 0.2.0에 막혀 강제 업데이트 화면을 보고, 0.2.0은 증인 목록·홈·상세 응답을 정상 파싱한다.
- [ ] 영구 보관 구매자의 증빙 사진은 종결 후 1년이 지나도 남아 있다(365일 자동 삭제 폐기).
...
- [ ] PENDING 작성자 삭제 외에는 약속 삭제 API가 없고, ACTIVE 이후 확정 기록은 숨기기만 가능하다.
...
- [ ] §10의 EC-A01~EC-L03 전 항목에 대한 테스트가 존재하고 통과한다.
- [ ] 동시성 항목(EC-B06, EC-C01, EC-C03, EC-E01, EC-F01)은 병렬 요청 테스트로 검증한다.
- [ ] 배치 J-01~J-06·J-08~J-11을 동일 시각으로 연속 2회 실행해도 결과가 동일하다(멱등). **J-07은 PO 결정으로 현재 범위 제외**.
- [ ] 이메일 주소가 앱·웹·Edge Function에서 수집·저장·발송되지 않고 EC-G03 경로가 비활성임을 확인한다.
...
| `기획/04_AI-Agent_코딩가이드.md` | 완료 (v1.0) | 확정 스택(Expo SDK 57 + Supabase + Firebase Hosting Spark), 저장소 구조, 기존 HTML/CSS 구현의 이식 규칙, 보안·RLS, 개발 순서 M0~M4 |
