...
| 4 | 종료일 | `end_date` | date nullable | 선택(무기한은 NULL) | KST 작성일 +30일까지 무료. 작성자 보상형 광고 1회마다 해당 약속 상한 +30일. 작성자 `promise_permanent_access`가 유효하면 NULL(종료일 없음) 또는 장기 날짜 가능. 발송·변경 요청 시 서버 재검증 | 없음 | `DRAFT` / 변경 합의(승인 시점 재검증) |
