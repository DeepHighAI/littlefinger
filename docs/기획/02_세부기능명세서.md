...
| 계정 탈퇴 | `status=WITHDRAWN`, `withdrawn_at` 기록 + **개인정보 비식별화**: `nickname`→"탈퇴한 사용자", `profile_image_url`→NULL, `email`→NULL, `kakao_id`→`sha256(kakao_id + ACCOUNT_ID_PEPPER)`로 대체. `user_id`와 참여·승인 이력은 유지해 **상대방의 기록 열람권 보존** |
