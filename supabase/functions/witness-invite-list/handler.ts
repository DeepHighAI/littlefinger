import { asWitnessInviteListResponse } from '../../../packages/shared/src/witness.ts';
...
export function createWitnessInviteListHandler(deps: Deps) {
  return async function handle(request: Request): Promise<Response> {
    ...
    try {
      ...
      const payload = asWitnessInviteListResponse(await deps.rpc('lf_witness_invite_list', {
        p_actor: actor,
        p_promise_id: promiseId,
      }));
      if (payload === null) throw new Error('INVALID_WITNESS_INVITE_LIST_RESPONSE');
